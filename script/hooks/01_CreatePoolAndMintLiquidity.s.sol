...
contract CreatePoolAndAddLiquidityScript is Script, Constants, Config {
    ...
    uint160 startingPrice = 4357548938284538567644917268480; // floor(sqrt(3025) * 2^96)

    // --- liquidity position configuration --- //
    uint256 public token0Amount = 1e3; // mWETH
    uint256 public token1Amount = 1e3 * 3000; // mDAI
    ...
    function run() external {
        ...
        params[1] = abi.encodeWithSelector(
            posm.modifyLiquidities.selector, abi.encode(actions, mintParams), block.timestamp + 600
        );
